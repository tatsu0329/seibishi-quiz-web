...
export default function StatsView({ onBack }: StatsViewProps) {
  ...
  const groupedBookmarks = useMemo(() => {
    ...
    bookmarks.forEach((bookmark) => {
      const key = `${bookmark.category.level}-${bookmark.category.fuelType}-${bookmark.category.year}`;
      if (!groups.has(key)) {
        groups.set(key, []);
      }
    ...
    return Array.from(groups.entries()).sort((a, b) => {
      const [aLevel, aFuel, aYear] = a[0].split("-");
      const [bLevel, bFuel, bYear] = b[0].split("-");
      ...
      // 燃料タイプでソート
      return aFuel.localeCompare(bFuel);
    });
  }, [bookmarks]);
    ...
    <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-blue-50 to-indigo-100 p-4 dark:from-gray-900 dark:to-gray-800">
      <div className="w-full max-w-4xl rounded-2xl bg-white p-8 shadow-xl dark:bg-gray-800">
        ...
        {bookmarks.length > 0 && (
          <div className="mt-8">
            <h2 className="mb-4 text-xl font-bold text-gray-800 dark:text-gray-100">
              ブックマーク ({bookmarks.length}問)
            </h2>
            <div className="space-y-4">
              {groupedBookmarks.map(([categoryKey, categoryBookmarks]) => {
                const [level, fuelType, year] = categoryKey.split("-");
                const category = { level, fuelType, year };
                ...
                return (
                  ...
                  >
                    ...
                    <button
                      ...
                      className="w-full p-4 text-left bg-gray-50 hover:bg-gray-100 dark:bg-gray-700 dark:hover:bg-gray-600 transition-colors"
                    >
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <div className="text-sm text-gray-600 dark:text-gray-400">
                              {categoryBookmarks.length}問のブックマーク
                            </div>
                          </div>
                        </div>
                    ...
                    {isCategoryExpanded && (
                      <div className="border-t border-gray-200 dark:border-gray-700">
                        <div className="p-3 space-y-3 bg-white dark:bg-gray-800 max-h-96 overflow-y-auto">
                          {categoryBookmarks.map((bookmark) => {
                            const bookmarkKey = `${categoryKey}-${bookmark.questionId}`;
